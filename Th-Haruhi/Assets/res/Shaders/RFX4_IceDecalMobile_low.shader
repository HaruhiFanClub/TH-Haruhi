...
Shader "KriptoFX/RFX4/Decal/IceMobile_low" {
	Properties{
		...
		CGPROGRAM
			...
			v2f vert(appdata_t v)
			{
				...
				o.lightDir =normalize( saturate( dot (float4 (0,1,0,0),normalDir)));
			...
			half4 c = tex * _Color* i.lightDir;
			half reflcol = dot(texCUBE(_Cube, i.refl*(bump*2-1)), 0.33);
			reflcol *= tex.a;
			reflcol = lerp(c, reflcol, i.uv_BumpFresnel.z);
			c.rgb =c*_Color + reflcol * _ReflectColor.rgb  * 4 * _Cutoff;
			c.a = tex.a * saturate(step(1 - tex.a, _Cutoff))*0.6;
			return c;
