...
public static class TextureEffectFactroy
{
    public delegate void CreatedNotify(TextureEffect effect);

    public class Holder
    {
        public TextureEffectDeploy Deploy;
        public List<Sprite> Resource;
        public Queue<TextureEffect> Queue;
        public float BackToPoolTime;
    }

    private const int OneCacheMax = 100;              //每个种类最多缓存数量

    private static readonly TableT<TextureEffectDeploy> EffectTab;
    ...
    {
        var effects = Resources.FindObjectsOfTypeAll<TextureEffect>();
