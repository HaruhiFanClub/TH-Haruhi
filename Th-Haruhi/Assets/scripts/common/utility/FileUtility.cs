...
public static class FileUtility
{
    ...
    public static byte[] ReadAllBytes(string path)
    {
        using (FileStream stream = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
        {
            ...
            while (count > 0)
            {
                int num4 = stream.Read(array, offset, count);
                if (num4 == 0)
                {
    ...
    public static string GetTextFromFile(string fileName)
    {
        ...
        try
        {
            stream = new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.Write);
        }
        catch (Exception)
        {
#if UNITY_EDITOR
            ...
            try
            {
                stream = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Write);
            }
            catch (Exception)
            {
                try
    ...
    public static string ComputeHash(string str)
    {
        ...
        {
            hash = BitConverter.ToString(md5.ComputeHash(Encoding.UTF8.GetBytes(str)), 4, 8);
            hash = hash.Replace("-", "");
        }
        return hash;
    }
