...
public static class GameEventCenter
{
    ...
    public static void Send(string _event, object argument = null)
    {
        ...
        {
            Delegate[] listeners = listenerSet.listener.GetInvocationList();
    ...
    {
        removes__.Clear();
        using (Dictionary<string, EventListenerSet>.Enumerator e = eventListeners.GetEnumerator())
        {
            while (e.MoveNext())
            {
                Delegate[] targetDelegates =
                    Array.FindAll(
